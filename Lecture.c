...
int gets(char fileName[257]){ //taille max des noms de fichiers sous unix
	...
	while(fgets(str[nbLigne], 160, fp)!=NULL) {
     ...
     if(nbLigne == 1){
     	...
     	{
     		printf("Transitions Etat %d : ",nbLigne-4);
     		puts(str[nbLigne-1]);
     	}
     }else{
...
int estLettre(char astr[60],char exception){ //verifie si c'est une lettre de l'alphabet
	int j = 0;
	while(astr[j] != '\n'){
...
int estChiffre(char astr[60],char exception){ //verifie si c'est un chiffre
	int j = 0;
	while(astr[j] != '\n'){
